...
@triton.jit
def _decode_e4m3fn_test_kernel(in_ptr, out_ptr, N: tl.constexpr):
    offs = tl.arange(0, N)
    u = tl.load(in_ptr + offs)
    tl.store(out_ptr + offs, _decode_e4m3fn(u))


def test_decode_e4m3fn_matches_pytorch_for_all_bytes():
    """Validate the manual E4M3FN→fp32 decode against PyTorch's native
    conversion for every uint8 input. NaN patterns (0x7F, 0xFF) are
    skipped from the equality check since the kernel decodes them
    numerically as ±480; their numeric output is asserted separately."""
    device = "cuda"
    bytes_ = torch.arange(256, dtype=torch.int32, device=device).to(torch.uint8)
    out = torch.empty(256, dtype=torch.float32, device=device)
    _decode_e4m3fn_test_kernel[(1,)](bytes_, out, N=256)

    ref = bytes_.view(torch.float8_e4m3fn).to(torch.float32)
    finite = ~torch.isnan(ref)
    torch.testing.assert_close(out[finite], ref[finite], atol=0.0, rtol=0.0)

    # +0 vs -0 is invisible to == comparison; check the sign bit directly.
    assert torch.signbit(out[[0x00, 0x80]]).tolist() == [False, True]
    # NaN bytes are decoded as ±480 (documented behavior).
    assert out[0x7F].item() == 480.0
    assert out[0xFF].item() == -480.0
