...
def sparse_attn_indexer(
    ...
    if has_prefill:
        ...
        for chunk in prefill_metadata.chunks:
            ...
            if _IS_DEEP_GEMM:
                ...
                # SM80/SM121: DeepGEMM unavailable. Use the Triton fallback,
                # which only supports the FP8 path (no FP4).
    ...
    if has_decode:
        ...
        if _IS_DEEP_GEMM:
            ...
            # SM80/SM121: DeepGEMM unavailable. Triton fallback only handles
            # the FP8 path.
            assert not use_fp4_cache, (
                "Triton sparse-MLA fallback does not support FP4 KV cache"
            )
            # Downstream topk reads only up to `seq_lens`, so we can size the
            # logits buffer to the active batch's longest seq instead of the
            # configured model max. Saves both a torch.full(-inf) over the
            # extra columns and the corresponding kernel programs that would
            # have early-exited.
